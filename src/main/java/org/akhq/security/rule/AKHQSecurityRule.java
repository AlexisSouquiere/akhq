...
public class AKHQSecurityRule extends AbstractSecurityRule<HttpRequest<?>> {
    ...
    public Publisher<SecurityRuleResult> check(HttpRequest<?> request, Authentication authentication) {
        ...
        if (authentication != null) {
            ...
            userGroups = unrollGroups(authentication, claimProvider);
        }
    ...
    public static List<Group> unrollGroups(Authentication authentication, ClaimProvider claimProvider) {
        Object decompressedGroups = decompressGroups(authentication);

        if ((decompressedGroups instanceof Map<?, ?>)) {
            return ((Map<String, List<?>>) decompressedGroups).values().stream()
                .flatMap(Collection::stream)
        ...
        return getClaimProviderGroups(providerName, (List<String>) decompressedGroups, authentication,
            claimProvider).values().stream()
            .flatMap(Collection::stream)
            .collect(Collectors.toList());
    }
